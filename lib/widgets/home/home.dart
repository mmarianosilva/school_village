...
class _HomeState extends State<Home> with WidgetsBindingObserver, DashboardScopeObserver {
  ...
  Future playAlarm() async {
    ...
    if ((role == 'security'  || role == 'admin' || role == 'super_admin' || role == 'district') && ((await SharedPreferences.getInstance()).getInt(Constants.lastAmberAlertTimestampKey) ?? 0) > DateTime.now().millisecondsSinceEpoch - 3600000) {
